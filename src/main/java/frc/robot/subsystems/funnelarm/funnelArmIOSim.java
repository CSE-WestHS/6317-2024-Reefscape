...
import frc.robot.subsystems.funnelarm.funnelArmConstants.funnelArmHardwareConfig;
import frc.robot.util.feedforwards.TunableSimpleMotorFeedforward;

public class funnelArmIOSim implements funnelArmIO {
  private final String name;

  private final funnelArmHardwareConfig config;
  ...
  public funnelArmIOSim(String name, funnelArmHardwareConfig config) {
    this.name = name;
    ...
    for (int i = 0; i < config.canIds().length; ++i) {
      System.out.println(config.canIds()[i]);
    }
    ...
    setGains(funnelArmConstants.REAL_GAINS); //sets sim gains for simulation
  }
  @Override
  public void updateInputs(funnelArmIOInputs inputs) {
    for (int i = 0; i < inputs.motorsConnected.length; ++i) {
      inputs.motorsConnected[i] = isConnected;
    }
  ...
  @Override
  public void setGains(funnelArmGains gains) {
